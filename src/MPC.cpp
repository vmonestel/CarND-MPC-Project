...
/* Define the timestep length and duration */
size_t N = 10;
double dt = 0.1;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
     ...
     * (state & actuators). The cost is stored in position 0 and this is the variable
     * that is minimized.
     */
    fg[0] = 0;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  /* We use 6 state variables [x,y,ψ,v,cte,eψ] */
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
   ...
   * timesteps * num_state_variables + (timesteps - 1) * num_actuators
   */
  size_t n_vars = N * 6 + (N - 1) * 2;
  /*
   * number of constraints
   * timesteps * num_state_variables
   */
